...
int main(){
	...
	system("Food Menu System");				// Set the console window title
	system("mode con: cols=88 lines=30");   // Set the console window size
	...
	head = NULL;
	insertfirst(1,"Faluda",23,120.23);
	...
	br(1); pre(4); fflush(stdin); // to clear the input buffer
		...
		// admin section
			...
			if(admin_panel_choice==1234567){
				...
				if(adminchoise==1){
					...
					fflush(stdin);
					scanf("%[^\n]s", ffoodname);	// to read a string until a newline character
					...
					char date[35]=__DATE__;	// expands to the current date in the format Mmm dd yyyy
...
void main_menu(){
	...
	cls();
	br(2); pre(3); echo(">> 1. Food Lists"); Sleep(400);
...
// displays colorful background
void ccolor(int clr){

	HANDLE  hConsole;	// HANDLE is a pointer to an unspecified type
	hConsole = GetStdHandle(STD_OUTPUT_HANDLE);	// This function retrieves a handle
	SetConsoleTextAttribute(hConsole, clr);	// sets the text and background color of the console output.
...
void loadingbar(void){
	...
	for (int i=15;i<=100;i+=5){
		...
		for (int j=0; j<i;j+=2){
			...
			ccolor(160+j);		// custom func to set the console text or background color.
			printf(" ");
			ccolor(26);
